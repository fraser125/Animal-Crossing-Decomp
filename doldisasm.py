    ...
    with open(path) as file:
        ...
        for line in lines:
            if line[0] == "!":
                ...
                # new source file
                fileName = os.path.splitext(line[1:])[0]
                file_changed = True
            else:
                # new symbol
                ...
                symbol = line[9:].strip()
                labels.add(addr)
                labelNames[addr] = symbol
                if (section_changed):
                    section_changed = False
                    sections.add(addr)
                    sectionNames[addr] = section_name
                if (file_changed):
                    file_changed = False
                    files.add(addr)
                    fileNames[addr] = fileName

# If symbol file was given, add symbols
if len(sys.argv) > 2:
    load_labels(sys.argv[2])
else:
    files.add(0x80003100)
    fileNames[0x80003100] = "asm"
...
current_file = "asm.s"
sys.stdout = open(current_file, 'w')
current_section = ""
...
def disasm_iter(offset, address, size, callback):
    ...
    while address < end:
        # Check if the current address is a new section
        if address in sections:
            if address in sectionNames:
                current_section = sectionNames[address]

        # Check if the current address has a file associated with it
        if address in files:
            if address in fileNames:
                current_file = fileNames[address] + ".s"
                sys.stdout = open(current_file, 'w')
                if (current_section != ""):
                        print(".include \"macros.inc\"\n")
                        print(".section " + current_section)
                        print("\n.org 0x" + '{:X}'.format(address))
